...
##################################################################################
# This script assumes init_cloud.py has already been run to configure Genie.
##################################################################################
...
job.command_arguments(
    ...
    "--class=com.epam.mvplatform.extractor.TableImport "
    "--properties=spark.driver.extraJavaOptions=-Dconfig.file=app.conf,spark.executor.extraJavaOptions=-Dconfig.file=app.conf "
    "--jars=gs://mvplatform-ci-artefacts/spark-app/genie-db-extractor/app.conf,"
    "gs://mvplatform-ci-artefacts/spark-app/genie-db-extractor/genie_table_conf.yaml,"
    "gs://mvplatform-ci-artefacts/jars/ingestion-data-extractors/snapshots/commons-0.1.0-SNAPSHOT.jar,"
    "gs://mvplatform-ci-artefacts/jars/ingestion-data-extractors/snapshots/db-extractor-0.1.0-SNAPSHOT.jar"
)
